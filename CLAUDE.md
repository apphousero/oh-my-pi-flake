...
The matrix is `x86_64-linux` and `aarch64-linux`; there is no macOS leg. `flake.nix` still exports `aarch64-darwin` and `update.sh` still refreshes the `omp-darwin-arm64` hash, so darwin is packaged but never built here. That is a deliberate trade: the darwin path is `fetchurl` + `install` with no `autoPatchelfHook`, its hash comes from the same published `SHA256SUMS.txt` as the Linux ones, and a macOS runner is the slowest leg in the fleet. The cost is real — a bad darwin asset surfaces at a user's `nix build`, not in CI — so keep `nix eval .#packages.aarch64-darwin.oh-my-pi.drvPath` in mind as the only thing that guards it.
